...
def run(repo_path, end_line=1, compute_type="float16", batch_size=16, threads=8, device="cuda", lang="en"):
    ...
    results_dir = f"{repo_path}/results/Faster-Whisper-bs_{batch_size}-{system}-{device}-{compute_type}-threads{threads}"
    os.makedirs(results_dir, exist_ok=True)

    model = WhisperModel(
        "large-v2", device, cpu_threads=threads, compute_type=compute_type)
    ...
    for fn in tqdm(files[:end_line], desc="audio trascribing"):
        ...
        normalize_result = normalizer(" ".join([_.strip()
                                                for _ in texts]))
